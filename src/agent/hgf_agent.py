...
class HGFAgent:
    ...
    def _configure_network(self):
        ...
        for i in range(len(self.preferences), 2 * len(self.preferences)):
            if "tonic_volatility" in self.hgf_network.attributes[i]:
                self.hgf_network.attributes[i]["tonic_volatility"] = (
    ...
    def get_decision_parameters(self):
        ...
        mu_belief = np.array(
            [
                self.hgf_network.node_trajectories[i + n_prefs]["expected_mean"][-1]
                for i in range(n_prefs)
            ]
        )
        prec_belief = np.array(
            [
                self.hgf_network.node_trajectories[i + n_prefs]["expected_precision"][
                    -1
                ]
                for i in range(n_prefs)
            ]
