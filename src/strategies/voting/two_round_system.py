...
from ..decision.hgf_strategy import (
    calculate_vote_from_params,
)  # Import the new pure function
...
class TwoRoundSystem(VotingSystem):
    ...
    def run_election(self, agents: list, environment, key) -> dict:
        ...
        votes_r1 = vmap_vote(
            ...
            agents_prec_beliefs,
            agents_mean_prefs,
            agents_precision_prefs,
            r1_candidate_params,
        )

        counts_r1 = np.bincount(
            np.array(votes_r1), minlength=len(environment.candidates)
        )
        proportions_r1 = (
            counts_r1 / np.sum(counts_r1)
            if np.sum(counts_r1) > 0
            else np.zeros_like(counts_r1)
        )
        ...
        if len(counts_r1) < 2 or np.max(proportions_r1) > 0.5:
            ...
            return {
                ...
                "round_1_proportions": proportions_r1,
                "round_2_proportions": None,
                "finalists_indices": None,
            }
        ...
        proportions_r2 = (
            counts_r2 / np.sum(counts_r2)
            if np.sum(counts_r2) > 0
            else np.zeros_like(counts_r2)
        )
        ...
        return {
            "winner_index": winner_original_idx,
            "round_1_proportions": proportions_r1,
            "round_2_proportions": proportions_r2,
            "finalists_indices": top2_indices,
        }
