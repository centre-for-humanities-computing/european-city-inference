...
def plot_distributions(x_range=(-6.0, 6.0, 102), figsize=(3, 5), save_path=Path().cwd().parent / "figures" / "fig_1_distributions.svg"):
    """
    ...
    preferences_list = [(4.0, .5), (0.0, 2.0), (4.0, .75)]
...
def plot_kl_divergences(figsize=(1, 5), save_path=Path().cwd().parent / "figures" / "fig_1_kl_divergences.svg"):
    """
...
def plot_time_series(observations, figsize=(9, 3), save_path=Path().cwd().parent / "figures" / "fig_1_time_series.svg"):
    """
    ...
    for i, color in zip(range(n_nodes), colors):
        ...
        axs[i].plot(time_steps, observations[:, i], color="grey", linewidth=0.75, zorder=-1)
        axs[i].set(ylim=(0, 1))
        axs[i].minorticks_on()
...
def plot_proportions(proportions, title='Scenario 2', figsize=(4, 6), colormap='tab20', alpha=0.7):
    """
    ...
    category_proportions = category_proportions.div(category_proportions.sum(axis=1), axis=0)  # Normalization

    # Plot the proportions
    category_proportions.plot(kind='area', stacked=True, figsize=figsize, colormap=colormap, alpha=alpha, linewidth=0)
...
def plot_trajectories(nodes_traje, preferences, pref_labels, line_styles=None, figsize=(18, 7)):
    """
    ...
    fig, axes = plt.subplots(1, len(preferences), figsize=figsize, sharey=True, facecolor='#f9f9f9')
    ...
    for idx, pref in enumerate(preferences):
        ...
        ax.grid(True, linestyle='--', alpha=0.4, color='#e0e0e0', linewidth=0.5)  # Very subtle grid
