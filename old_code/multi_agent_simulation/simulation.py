...
class Agent:
    ...
    def _create_network(self):
        ...
        """
        network = Network().add_nodes(
            kind="binary-state", n_nodes=self.simulation.n_nodes
        )
    ...
    def configure(self):
        ...
        if self.preference_normal is None:
            ...
            precision = np.random.gamma(
                shape=2.0, scale=1.0, size=self.simulation.n_nodes
            )
        ...
        if self.preferences is not None:
            ...
            self.network.attributes[-1]["preference"] = np.clip(
                sampled_preferences, 0, 1
            )

        if self.beta_params is not None:
            self.network.attributes[-1]["preference_beta"] = self.beta_params

        self.network.attributes[2]["tonic_volatility"] = (
            self.tonic_volatility + np.random.normal(0, 0.5)
        )
        self.network.attributes[3]["tonic_volatility"] = (
            self.tonic_volatility + np.random.normal(0, 0.5)
        )
...
class Voter(Agent):
    def __init__(
        ...
        """
        super().__init__(
            simulation, tonic_volatility, preferences, beta_params, preference_normal
        )
    ...
    def calculate_dissatisfaction(self):
        ...
        for i in range(n_nodes):
            if i < len(self.network.node_trajectories):
                ...
                prec_belief = self.network.node_trajectories[i]["expected_precision"][
                    -1
                ]
    ...
    def vote(self, candidates):
        ...
        for i, candidate in enumerate(candidates):
            ...
            candidate_mean_pref, candidate_precision_pref = (
                candidate.network.attributes[-1]["preference_normal"]
            )
            ...
            for node in range(n_nodes):
                if node < len(candidate_mean_pref):
                    print(
                        f"  Node {node}: mean = {candidate_mean_pref[node]:.2f}, precision = {candidate_precision_pref[node]:.2f}"
                    )
            ...
            for node in range(n_nodes):
                if node < len(candidate_mean_pref):
                    kl = (
                        ...
                        + (
                            var_pref[node]
                            + (mean_pref[node] - candidate_mean_pref[node]) ** 2
                        )
                        / (2 * candidate_var_pref[node])
                        - 0.5
                    )
            ...
            print(
                f"Total expected dissatisfaction with Candidate {i}: {total_expected_dissatisfaction:.2f}"
            )
        ...
        self.voting_history.append(
            {
                ...
                "expected_dissatisfaction_with_chosen": min_total_dissatisfaction
                if best_candidate
                else float("inf"),
            }
        )
...
class MultiAgentSimulation:
    def __init__(
        self, n_steps=100, n_nodes=3, n_agents_per_population=5, n_candidates=None
    ):
        self.n_steps = n_steps
        self.n_nodes = n_nodes
        self.n_agents_per_population = n_agents_per_population
        self.n_candidates = (
            n_candidates if n_candidates is not None else n_agents_per_population
        )
        ...
        if scenario == 1:
            ...
            for node in range(self.n_nodes):
                ...
                if node < len(node_beta_params):
                    beta_params = node_beta_params[node][
                        0
                    ]  # Use only the first phase parameters for stability
                else:
                    # Default values if not specified for this node
                    beta_params = (5, 1)

                # Generate stable observations for the entire duration
                complete_observation = np.random.beta(
                    a=beta_params[0], b=beta_params[1], size=self.n_steps
                )
            ...
            if shock_pattern is None or shock_pattern == "phase":
                ...
                for node in range(self.n_nodes):
                    ...
                    observation_phase1 = np.random.beta(
                        a=phase1_params[0], b=phase1_params[1], size=phase1_end
                    )
                    observation_phase2 = np.random.beta(
                        a=phase2_params[0],
                        b=phase2_params[1],
                        size=phase2_end - phase1_end,
                    )
                    observation_phase3 = np.random.beta(
                        a=phase3_params[0],
                        b=phase3_params[1],
                        size=self.n_steps - phase2_end,
                    )

                    complete_observation = np.concatenate(
                        (observation_phase1, observation_phase2, observation_phase3)
                    )
                    node_observations.append(complete_observation)
                ...
                if recovery_time is None:
                    ...
                    raise ValueError(
                        "recovery_time must be between shock_time and n_steps-1"
                    )
                ...
                for node in range(self.n_nodes):
                    ...
                    observation_pre_shock = np.random.beta(
                        a=phase1_params[0], b=phase1_params[1], size=shock_time
                    )
                    observation_post_shock = np.random.beta(
                        a=phase2_params[0],
                        b=phase2_params[1],
                        size=recovery_time - shock_time,
                    )
                    observation_recovery = np.random.beta(
                        a=phase3_params[0],
                        b=phase3_params[1],
                    ...
                    complete_observation = np.concatenate(
                        (
                            observation_pre_shock,
                            observation_post_shock,
                ...
                for node in range(self.n_nodes):
                    ...
                    for t in range(
                        recovery_time // 2
                    ):  # First half of recovery_time is phase 1
                    ...
                    for t in range(trend_start, trend_end):
                        ...
                        alpha = (
                            phase1_params[0] * (1 - weight) + phase2_params[0] * weight
                        )
                        beta_param = (
                    ...
                    for t in range(recovery_start, recovery_end):
                        ...
                        alpha = (
                            phase2_params[0] * (1 - weight) + phase1_params[0] * weight
                        )
                        beta_param = (
    ...
    def run_simulation(
        ...
        population_1_preferences = np.array(
            [0.1, 0.4]
        )  # Exemple de préférences pour la Population 1
        population_2_preferences = np.array(
            [0.8, 0.1]
        )  # Exemple de préférences pour la Population 2
        ...
        population_1 = [
            self.create_agent(
                "voter", volatility_pop1, population_1_preferences, beta_params_pop1
            )
        ...
        population_2 = [
            self.create_agent(
                "voter", volatility_pop2, population_2_preferences, beta_params_pop2
            )
            for _ in range(n_voters_per_population)
        ]
        ...
        for i in range(n_candidates):
            ...
            mean = np.random.uniform(
                0, 1, size=self.n_nodes
            )  # Moyenne aléatoire entre 0 et 1 pour chaque nœud
            precision = np.random.gamma(
                shape=2.0, scale=1.0, size=self.n_nodes
            )  # Précision aléatoire pour chaque nœud
            preference_normal = (mean, precision)

            # Créer le candidat avec ces préférences
            candidate = self.create_agent(
                "candidate",
            ...
            for node in range(self.n_nodes):
                print(
                    f"  Node {node}: mean = {mean[node]:.2f}, precision = {precision[node]:.2f}"
                )
        ...
        for i, agent in enumerate(self.agents):
            self.agent_networks[i].input_data(
                input_data=self.observations[i], time_steps=np.ones(self.n_steps)
            )
    ...
    def simulate_voting(self):
        ...
        for voter in voters:
            ...
            if chosen_candidate:
                ...
                print(
                    f"Voter {voter_index} (Population {population}) voted for Candidate {candidate_index}"
                )
            else:
                votes.append(None)
                voter_index = voters.index(voter)
    ...
    def plot_trajectories(self):
        ...
        colormap = cm.get_cmap(
            "tab20", 20
        )  # Supports jusqu'à 20 populations distinctes
        ...
        for i in range(self.n_nodes):
            ...
            for j, agent in enumerate(self.agents[:total_voters]):
                ...
                (line,) = ax.plot(
                    self.agent_networks[j].node_trajectories[i]["expected_mean"],
                    color=color,
                    linestyle=linestyle,
                    linewidth=1.5,
                    alpha=0.7,
                ...
                if (
                    len(added_agents) < 12
                    and (population, agent_in_pop) not in added_agents
                ):
            ...
            if len(self.agents[:total_voters]) > 12:
                ...
                handles
            ):  # Ajouter la légende uniquement s'il y a des handles à afficher
                ax.legend(
                    handles=handles,
                    labels=labels,
                    title="Agents",
    ...
    def plot_observations(self):
        ...
        colormap = cm.get_cmap(
            "tab10", 2
        )  # Two distinct colors for the two populations
    ...
    def plot_agent_data(self):
        ...
        for i, agent in enumerate(self.agents):
            ...
            agent_last_expected_mean = self.agent_networks[i].node_trajectories[0][
                "expected_mean"
            ][time_step]
            ...
            if not isinstance(preferences, (np.ndarray, list)):
                preferences = []
            preferences = (
                np.array(preferences) if isinstance(preferences, list) else preferences
            )
            ...
            n_nodes_to_plot = min(
                self.n_nodes, surprises.shape[1] if i < surprises.shape[0] else 0
            )
            ...
            for j in range(n_nodes_to_plot):
                ...
                if j < surprises.shape[1] and i < surprises.shape[0]:
                    ...
                    surprise_values.append(
                        0
                    )  # Valeur par défaut si l'index est hors limites
            ...
            axs[i].axhline(
                y=agent_last_expected_mean,
                color="green",
                linestyle="--",
                label="Last Expected Mean",
            )

            axs[i].set_title(
                f"Agent {i + 1} (Population {population}): Preferences, Surprise Values, and Last Expected Mean"
            )
            axs[i].set_xticks(x)
    ...
    def get_agent_surprises(self, time_step):
        ...
        for i, agent_network in enumerate(self.agent_networks):
            ...
            for j in range(self.n_nodes):
                ...
                if time_step >= len(
                    agent_network.node_trajectories[j]["expected_mean"]
                ):
                    continue  # Skip if time_step is out of range
                expected_mean = agent_network.node_trajectories[j]["expected_mean"][
                    time_step
                ]
                preferences = agent.network.attributes[-1].get("preference", [])
    ...
    @staticmethod
    def plot_preference_normals_all_agents(
        simulation, sigmoid=False, plot_beliefs=False
    ):
        """
        ...
        for idx, agent in enumerate(simulation.agents):
            mean_pref, precision_pref = agent.network.attributes[-1][
                "preference_normal"
            ]
            std_pref = 1 / np.sqrt(precision_pref)
            ...
            for i in range(n_dims):
                x_pref = np.linspace(
                    mean_pref[i] - 4 * std_pref[i], mean_pref[i] + 4 * std_pref[i], 500
                )
                ...
                if not sigmoid:
                    ax.plot(
                        x_pref,
                        pdf_pref,
                        label=f"Pref Dim {i}: N({mean_pref[i]:.2f}, {std_pref[i] ** 2:.3f})",
                    )
                else:
                    y_pref = expit(x_pref)
                    ...
                    ax.plot(
                        y_pref,
                        pdf_y_pref,
                        label=f"Pref Dim {i}: Sigmoid N({mean_pref[i]:.2f}, {std_pref[i] ** 2:.3f})",
                    )

            if plot_beliefs:
                mean_belief = agent.network.node_trajectories[1]["expected_mean"][-1]
                precision_belief = agent.network.node_trajectories[1][
                    "expected_precision"
                ][-1]
                std_belief = 1 / np.sqrt(precision_belief)

                x_belief = np.linspace(
                    mean_belief - 4 * std_belief, mean_belief + 4 * std_belief, 500
                )
                ...
                if not sigmoid:
                    ax.plot(
                        x_belief,
                        pdf_belief,
                        label=f"Belief: N({mean_belief:.2f}, {std_belief**2:.3f})",
                        linestyle="--",
                    )
                else:
                    ...
                    ax.plot(
                        y_belief,
                        pdf_y_belief,
                        label=f"Belief: Sigmoid N({mean_belief:.2f}, {std_belief**2:.3f})",
                        linestyle="--",
                    )
    ...
    def compute_kl_divergence_preferences(simulation, node_idx=1):
        ...
        for agent in simulation.agents:
            ...
            kl = np.sum(
                np.log(std_2 / std_1) + (var_1 + (mu_1 - mu_2) ** 2) / (2 * var_2) - 0.5
            )
